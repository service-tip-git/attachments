    ...
    try {
      ...
      const cleanedMetadata = Object.fromEntries(
        Object.entries(metadata).filter(([k, v]) => {
          const isSerializable = v === null || typeof v !== 'object' || v instanceof String || typeof v.toJSON === 'function';
          if (!isSerializable) {
      ...
      // Optional: Debug full cleaned metadata
      DEBUG?.('[Cleaned Metadata for DB]', cleanedMetadata);

      const dbStorePromise = super.put(attachments, cleanedMetadata, null, isDraftEnabled)
        .catch(err => {
