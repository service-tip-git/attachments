  ...
  async put(attachments, data, isDraftEnabled, _content, req) {
    try {
      ...
      const metadata = Object.fromEntries(
        Object.entries(data).filter(([key]) => !['content', 'Body'].includes(key))
      );
      ...
      // Log and sanitize metadata before DB call
      const cleanedMetadata = Object.fromEntries(
        Object.entries(metadata).filter(([k, v]) => {
          ...
          if (!isSerializable || typeof v?.pipe === 'function') {
            console.warn(`[Warning] Removing non-serializable field "${k}" from metadata before DB insert. Type: ${typeof v}`);
            return false;
          }
      ...
      // Deep debug log
      try {
        const logStr = JSON.stringify(cleanedMetadata);
        DEBUG?.('[Cleaned Metadata for DB]', logStr);
      } catch (err) {
        console.error('[Cleaned Metadata could not be stringified]', err, cleanedMetadata);
      }

      // Log before DB call
      console.log(`[DB WRITE] About to call super.put() with metadata:`, cleanedMetadata);

      let dbStorePromise = (async () => {
        try {
          const result = await super.put(attachments, cleanedMetadata, null, isDraftEnabled);
          console.log(`[DB WRITE SUCCESS] super.put() returned successfully for ID: ${cleanedMetadata.ID}`);
          return result;
        } catch (err) {
          console.error(`[DB Error] Failed to store metadata for ID: ${cleanedMetadata.ID}`, err);
          throw new Error('Failed to store attachment metadata: ' + err.message);
        }
      })();
