...
const smAuthClient = require('./utils/SMClient.js')
...
const OBJECT_STORE_PLAN = "OBJECT_STORE";
...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async createClientS3(tenantID) {
    try {
      ...
      const token = await smAuthClient.fetchToken(url, clientid, clientsecret, certificate, key, certurl);

      const objectStoreCreds = await smAuthClient.getServiceBindingCredentials(tenantID, OBJECT_STORE_PLAN, sm_url, token);
