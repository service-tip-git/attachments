...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async createClientS3(tenantID) {
    LOG.info(`[S3 Upload] Creating S3 client for tenant ${tenantID}`);
    try {
      ...
      this.client = s3ClientsCache[tenantID].client;
      this.bucket = s3ClientsCache[tenantID].bucket;
      LOG.info(`[S3 Upload] Created S3 client for tenant ${tenantID}`);
    } catch (error) {
      // eslint-disable-next-line no-console
      LOG.error(`Creation of S3 client for tenant ${tenantID} failed`, error);
    }
  }

  async put(attachments, data, isDraftEnabled, _content, req) {
    LOG.info(`[S3 Upload] Executing put for file in ${attachments}`);
    ...
    try {
      ...
      LOG.info(`[S3 Upload] File uploaded successfully using put to ${this.bucket}`);
    } catch (err) {
  ...
  async updateContentHandler(req, next) {
    LOG.info(`[S3 Upload] Uploading file using updateContentHandler for ${req.target.name}`);
    // Check separate object store instances
    if (separateObjectStore) {
      const tenantID = req.tenant;
      await this.createClientS3(tenantID);
    ...
    if (req?.data?.content) {
      ...
      if (response?.url) {
        ...
        LOG.info(`[S3 Upload] Uploaded file using updateContentHandler for ${req.target.name}`);
      }
    } else if (req?.data?.note) {
      const key = { ID: req.data.ID };
      await super.update(req.target, key, { note: req.data.note });
      LOG.info(`[S3 Upload] Updated file upload with note for ${req.target.name}`);
    } else {
      next();
    }
  }
