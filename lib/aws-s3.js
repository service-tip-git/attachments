...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async createClientS3(tenantID) {
    console.log(`[S3 Upload] Creating S3 client for tenant ${tenantID}`);
    try {
      ...
      this.client = s3ClientsCache[tenantID].client;
      this.bucket = s3ClientsCache[tenantID].bucket;
      console.log(`[S3 Upload] Credentials for ${tenantID}:`, this.client, this.bucket, objectStoreCreds.credentials.access_key_id, objectStoreCreds.credentials.secret_access_key);

      console.log(`[S3 Upload] Created S3 client for tenant ${tenantID}`);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.log(`Creation of S3 client for tenant ${tenantID} failed`, error);
    }
  }

  async put(attachments, data, isDraftEnabled, _content, req) {
    console.log(`[S3 Upload] Executing put for file in ${attachments}`);
    ...
    try {
      ...
      console.log(`[S3 Upload] File uploaded successfully using put to ${this.bucket}`);
    } catch (err) {
  ...
  async get(attachments, keys, req = {}) {
    console.log(`[S3 Upload] Executing get for file in ${attachments}`);
    // Check separate object store instances
    if (separateObjectStore) {
      const tenantID = req.tenant;
      await this.createClientS3(tenantID);
  ...
  async updateContentHandler(req, next) {
    console.log(`[S3 Upload] Uploading file using updateContentHandler for ${req.target.name}`);
    // Check separate object store instances
    if (separateObjectStore) {
      const tenantID = req.tenant;
      await this.createClientS3(tenantID);
    ...
    if (req?.data?.content) {
      ...
      if (response?.url) {
        ...
        console.log(`[S3 Upload] Uploaded file using updateContentHandler for ${req.target.name}`);
      }
    } else if (req?.data?.note) {
      const key = { ID: req.data.ID };
      await super.update(req.target, key, { note: req.data.note });
      console.log(`[S3 Upload] Updated file upload with note for ${req.target.name}`);
    } else {
      next();
    }
  }
