...
function readAttachment(Key){
  const token = getSDMCredentials();
  const objectId = readDocument(Key, token);
  console.log("Check 3 ",objectId)
}



function readDocument(Key,token) {
  const { repositoryId } = getConfigurations();
  const creds = this.options.credentials;
  this.sdmurl = creds.uri;
  const documentReadURL = this.sdmurl + "browser/" + repositoryId + + "/root?objectID=/" + Key + "&cmisselector=content";  

  axios.get(documentReadURL, {
      headers: {
