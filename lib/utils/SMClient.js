...
async function getServiceBindingCredentials(tenantID, plan, sm_url, token) {
  try {
    ...
    console.log(`Error fetching object store credentials: ${error.message}`);
  }
}
...
async function fetchTokenWithClientSecret(url, clientid, clientsecret) {
  try {
    ...
    console.log(`Error fetching token for client credentials: ${error.message}`);
    throw error;
  }
}
...
async function fetchTokenWithMTLS(certURL, clientid, certificate, key) {
  try {
    ...
    console.log(`Error fetching token with MTLS: ${error.message}`);
    throw error;
  }
}
