...
cds.once("served", async function registerPluginHandlers () {
  ...
  async function nonDraftUpload(req, target) {
    if (req?.content?.url?.endsWith("/content")) {
      ...
      const contentStream = req.content?.body || req.content;
      if (!contentStream || typeof contentStream.pipe !== 'function') {
        console.error('[Attachments] No valid stream found in req.content');
        return;
      }

      // Only pass ID and content stream — nothing else
      const payload = {
        ID: attachmentID,
        content: contentStream
      };

      console.log(`[nonDraftUpload] Sending payload to nonDraftHandler for ID: ${attachmentID}`);
      AttachmentsSrv.nonDraftHandler(target, payload);
    }
  }
