...
const DEBUG = LOG._debug ? LOG.debug : (...args) => {}
const attachmentIDRegex = /\/\w+\(.*ID=([0-9a-fA-F-]{36})/

function handleError(err, context) {
  LOG.error(`${context} failed: ${err.message}`, { stack: err.stack });
  throw err;
}
...
cds.once("served", async function registerPluginHandlers () {
  ...
  try {
    ...
    if (!AttachmentsSrv) {
      LOG.error("Attachments service connection failed");
      return;
    }
  } catch (err) {
    handleError(err, "Connecting to attachments service");
    return;
  }
  ...
  function onPrepareAttachment(req) {
    try {
      if (!req?.data) throw new Error("Missing request data in onPrepareAttachment"); //req.data and req.data.filename are not present
      ...
      const ext = req.data.filename ? extname(req.data.filename).toLowerCase().slice(1) : "";
      req.data.mimeType = Ext2MimeTypes[ext] || "application/octet-stream";
    } catch (err) {
      handleError(err, "onPrepareAttachment");
    }
  }
  ...
  async function validateAttachment(req) {
    ...
    try {
      ...
      if (req?.req?.url?.endsWith("/content")) {
        const match = req.req.url.match(attachmentIDRegex);
        if (!match) throw new Error("Attachment ID not found in URL");
        const attachmentID = match[1];

        let status;
        try {
          status = await AttachmentsSrv.getStatus(req.target, { ID: attachmentID });
        } catch (err) {
          handleError(err, "Fetching attachment status");
        }
      ...
      handleError(err, "validateAttachment");
    }
  }
  ...
  async function readAttachment([attachment], req) {
    try {
      ...
      const keys = { ID: match[1] };
      const { target } = req; // Restored from original

      DEBUG?.(`readAttachment: Fetching content for attachment ID ${keys.ID}, target: ${target?.name || target}`);

      try {
        attachment.content = await AttachmentsSrv.get(target, keys, req); //Dependency -> sending req object for usage in SDM plugin
      } catch (err) {
        handleError(err, "Fetching attachment content");
      }
    } catch (err) {
      handleError(err, "readAttachment");
    }
  }
