...
function unfoldModel (csn) {
  console.warn("[DEBUG Logs] - Where am I? unfoldModel.")
  const meta = csn.meta ??= {}
...
cds.once("served", async function registerPluginHandlers () {
  console.warn("[DEBUG Logs] - Where am I? registerPluginHandlers.")
  if (!("sap.attachments.Attachments" in cds.model.definitions)) {
    console.warn(`This is the cds.model.definitions:`, JSON.stringify(cds.model.definitions))
    console.warn("[DEBUG Logs] - Attachments plugin is not in cds.model.definitions. Skipping handlers registration.")
    return
  }
  const AttachmentsSrv = await cds.connect.to("attachments")
  // Searching all associations to attachments to add respective handlers
  for (let srv of cds.services) {
    console.log("[DEBUG Logs] - Serving attachments plugin handlers...");
    if (srv instanceof cds.ApplicationService) {
      Object.values(srv.entities).forEach((entity) => {
        ...
        for (let elementName in entity.elements) {
          ...
          } else if (target?.["@_is_media_data"]) {

            console.log(`[DEBUG Logs] - Serving attachments plugin handlers on non draft entity - target: ${target.name}`);
          // Plug-in Customization via patch - enabling PUT and DELETE handlers for attachments on non draft entities
            srv.before("DELETE", target, (req) => validateAttachmentSize(req))
            DEBUG?.("serving attachments for:", target.name)

            srv.before("READ", target, validateAttachment)

            srv.after("READ", target, readAttachment)

            srv.before("PUT", target, (req) => validateAttachmentSize(req))

            AttachmentsSrv.registerUpdateHandlers(srv, entity, target)

            srv.before('CREATE', target, req => {
              req.data.url = cds.utils.uuid()
              const isMultitenacyEnabled = !!cds.env.requires.multitenancy;
              const objectStoreKind = cds.env.requires?.attachments?.objectStore?.kind;
              if (isMultitenacyEnabled && objectStoreKind === "shared") {
