...
module.exports = class AttachmentsService extends cds.Service {
  ...
  async put(attachments, data, _content, isDraftEnabled=true) {
    ...
    if (this.kind === 'db') {
      await Promise.all(
        data.map(async (d) => {
          console.log(`Scanning attachment ID: ${d.ID}`);
          await scanRequest(attachments, { ID: d.ID });
          console.log(`Scan completed for ID: ${d.ID}`);
        })
      );
    }
  ...
  async get(attachments, keys, req = {}) {
    ...
    console.log("Downloading attachment for", attachments.name, keys);
  ...
  async update(Attachments, key, data) {
    console.log("Updating attachment for", Attachments.name, key)
