...
const _serviceManagerRequest = async (sm_url, method, path, token, params = {}) => {
    try {
        ...
        console.log(`Error fetching data from service manager: ${error.message}`);
    }
};
...
const _validateSMCredentials = ({ sm_url, url, clientid, clientsecret, certificate, key }) => {
  ...
  if (!clientid || !clientsecret) {
    console.log("Client credentials not found. Falling back to MTLS if certificate and key are provided.");
    if (!certificate || !key) {
...
const _fetchToken = async (url, clientid, clientsecret, certificate, key) => {
  try {
    ...
    if (clientid && clientsecret) {
      console.log("Using OAuth client credentials to fetch token.");
    ...
    console.log("Client credentials missing, falling back to MTLS...");

    // Case 2: MTLS Flow
    if (certificate && key) {
      console.log("Using MTLS certificate/key to fetch token.");
    ...
    console.log(`Error fetching token from service manager: ${error.message}`);
    throw error;
  }
};
...
const _getOfferingID = async (sm_url, token) => {
    ...
    if (!offeringID) console.log('Object store service offering not found');
    return offeringID;
}
...
const _getPlanID = async (sm_url, token, offeringID) => {
  ...
  for (const planName of supportedPlans) {
    try {
      ...
      if (plan?.id) {
        console.log(`Using object store plan "${planName}" with ID: ${plan.id}`);
        return plan.id;
      }
    } catch (error) {
      console.log(`Failed to fetch plan "${planName}": ${error.message}`);
    }
  }
  console.log(
    ...
    try {
        ...
        while (!isReady) {
            ...
            if (Date.now() - startTime > ASYNC_TIMEOUT) {
                console.log('Timed out waiting for service instance to be ready');
            }

            if (instanceStatus.data.state === STATE.FAILED) {
                console.log('Service instance creation failed');
            }
        }
    } catch (error) {
        console.log(`Error polling for object store instance readiness: ${error.message}`);
    }
};
...
const _deleteBinding = async (sm_url, bindingID, token) => {
    if (bindingID) {
        try {
            ...
            console.log('Error deleting binding:', error.message);
        }
    } else {
