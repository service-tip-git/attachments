...
cds.on('listening', async () => {
    ...
    if (profile === 'mtx-sidecar') {
        ...
        } else if (separateObjectStore === "shared") {
            ds.after('unsubscribe', async (_, req) => {
                const { tenant } = req.data;
                //getting objectstore instance credentials
                const objectWithKey = [];
                ...
                try {
                    ...
                    //console.log("Response for list of objects", paginator);

                    for await (const page of paginator) {
                        // console.log("Page from paginator:", page);
                        const filteredKeys = page.Contents
                            .filter((o) => o.Key.includes(tenant))
                            .map((o) => ({ Key: o.Key }));

                        if (filteredKeys.length > 0) {
                            objectWithKey.push(filteredKeys);
                        }
                    }

                    // console.log("Object with key values only",objectWithKey);
                    const flatObjectKeys = objectWithKey.flat();
                    // console.log("Printing flat object with key",objectWithKey);
                    ...
                    try {
                        ...
                        console.error("Error deleting the objects in S3 bucket", error.message);
                    }

                } catch (error) {
                    console.error(`Error listing objets from S3 bucket: ${error.message}`);
